...
resultPath = '/home/rana/Thesis/DrQA/upal/_Results/LRCN/'

USER_THRESH = 15
SEQ_LEN_ACC = 125
SEQ_LEN_GYRO = 200
ACC_MODEL_PATH = '/home/rana/Thesis/DrQA/upal/_Models/'
ACC_MODEL_NAME = 'LRCNAccRawNpy_Conv5_25_5_L200_D200'
GYRO_MODEL_PATH = '/home/rana/Thesis/DrQA/upal/_Models/'
GYRO_MODEL_NAME = 'LRCNGyroRawNpy_Conv5_25_5_L200_D200'

# WEIGHTS ARE CURRENTLY GIVEN BY PRECISION
ACC_FUSION_WEIGHTS = np.array([0.70,0.25,0.26,0.28])
GYRO_FUSION_WEIGHTS = np.array([0.59,0.25,0.20,0.25])
fused_weight_sum = ACC_FUSION_WEIGHTS+GYRO_FUSION_WEIGHTS
print("fused_weight_sum: ", fused_weight_sum)

##########################################################################################################################
def reshapeTimeDist(seq,n_steps,n_features,SEQ_LEN):
    # reshape for timedistributed
    n_length = int(SEQ_LEN / n_steps)
    seq = seq.reshape(seq.shape[0], n_steps, n_length, n_features)
    return seq
...
for user_id in common_users:
    ...
    for common_user_t in common_users_timestamps:
        ...
        # reshape for timedistributed
        acc_sequences = reshapeTimeDist(acc_sequences, n_steps=5, n_features=acc_sequences.shape[2], SEQ_LEN=SEQ_LEN_ACC)
        gyro_sequences = reshapeTimeDist(gyro_sequences, n_steps=8, n_features=gyro_sequences.shape[2],SEQ_LEN=SEQ_LEN_GYRO)
...
ExtraSensoryHelperFunctions.save_dict_file(resultPath+'LRCN_Acc_Gyro_Fusion_ROC_results.txt',metrics_texts)
...
# plt.show()
plt.savefig(resultPath+'LRCN_Acc_Gyro_Fusion_ROC.png')