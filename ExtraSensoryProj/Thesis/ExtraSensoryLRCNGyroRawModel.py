...
from tensorflow.keras.layers import LSTM,Bidirectional, Dense, Conv1D, MaxPooling1D, AveragePooling1D,Dropout, Flatten,TimeDistributed
...
rawDataFolder = '/home/rana/Software&Data/Data/Upal/no_timestamp/phn_gyro_norm_dropna_5sec/'
...
resultPath = '/home/rana/Thesis/DrQA/upal/_Results/LRCN/Gyro/'

LOOP_COUNT = 3
SAVE_MODEL_NAME = 'LRCNGyroRawNpy_Conv5_25_5_L200_D200'
# 40HZ, THAT MEANS 5 SECS of data = 200
SEQ_LEN = 200
user_threshold =60
...
for path in Path(rawDataFolder).glob('**/*.npy'):
    ...
    # load the preprocessed npy data in dataframe
    # skip if it's empty
    ...
    # sequences = sequences.append(np_features)
    # sequences_labels = sequences_labels.append(np_labels_bin)
...
print('n_sequence,n_features: ', sequences.shape)
...
# model 1
model = Sequential()
model.add(TimeDistributed(Conv1D(filters=25, kernel_size=5,activation='relu'),input_shape=(None,n_length,n_features)))
model.add(TimeDistributed(Conv1D(filters=25, kernel_size=5,activation='relu')))
model.add(TimeDistributed(Conv1D(filters=25, kernel_size=5,activation='relu')))
model.add(TimeDistributed(Conv1D(filters=25, kernel_size=5,activation='relu')))
model.add(TimeDistributed(Conv1D(filters=25, kernel_size=5,activation='relu')))
